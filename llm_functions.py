...
def get_answer(question, model_name: str, judge: bool = False, prompt_template: str = None):
    ...
    for i, key in enumerate(api_keys):
        ...
        try:
            ...
            if is_last_key:
                last_exception = e # Store the exception if it happens on the last key
                break
            else:
                 # For non-rate-limit errors on other keys, you might want to stop sooner
                raise e
            
    # If the loop completes without a successful call, raise the last known exception
    if last_exception:
        print("[ERROR] All API keys have failed.")
